import type { ReactElement } from 'react'
import { Section } from './Section'
import './ContactSection.css'
import { anchorFor } from '../utils/site-routes'
import { useT } from '../i18n/useLanguage'
import type { ChannelKind } from '../data/public-content'

type ContactChannel = {
  /** What the visitor reads, in Spanish: "Instagram", "Correo", and so on. */
  label: string
  /** Where it goes: a `mailto:` address or the profile's URL. */
  href: string
  /** Optional emoji, shown only when the destination has no drawn icon. */
  glyph?: string
  /** Which drawn icon the destination gets; null falls back to the emoji. */
  kind?: ChannelKind
  /** The visible address under the name: the @handle or the mail address. */
  detail?: string | null
}

type ContactSectionProps = {
  /**
   * The league's channels. The functional document asks for an email address
   * and the social accounts; neither is recorded in any source handed over, so
   * nothing is hardcoded here and no address is invented.
   */
  channels?: ContactChannel[]
}

/**
 * The recognised destinations, drawn rather than typed: the first Instagram
 * row shipped wearing two beer mugs because the panel's glyph field renders
 * whatever an operator writes, and a brand people recognise by its mark should
 * carry that mark. Stroked outlines except WhatsApp, whose mark only reads as
 * itself filled.
 */
const CHANNEL_ICONS: Readonly<
  Record<Exclude<ChannelKind, null>, ReactElement>
> = {
  instagram: (
    <>
      <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
      <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
      <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
    </>
  ),
  mail: (
    <>
      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
      <polyline points="22,6 12,13 2,6" />
    </>
  ),
  facebook: (
    <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
  ),
  whatsapp: (
    <path
      fill="currentColor"
      stroke="none"
      d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"
    />
  ),
}

function ChannelIcon({ kind, glyph }: Pick<ContactChannel, 'kind' | 'glyph'>) {
  if (kind == null)
    return (
      <span className="contact__glyph" aria-hidden="true">
        {glyph ?? '✉'}
      </span>
    )
  return (
    <svg
      className="contact__icon"
      data-icon={kind}
      aria-hidden="true"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      {CHANNEL_ICONS[kind]}
    </svg>
  )
}

/**
 * Contact. The section the functional document asks for and the reference site
 * never had.
 */
export function ContactSection({ channels = [] }: ContactSectionProps) {
  const t = useT()
  return (
    <Section
      id={anchorFor('contacto')}
      eyebrow={t('Escribinos')}
      title={t('Contacto')}
    >
      {channels.length === 0 ? (
        <p className="contact__empty">
          {t('Todavía no hay canales de contacto publicados.')}
        </p>
      ) : (
        <ul className="contact">
          {channels.map((channel) => (
            <li key={channel.href}>
              <a className="contact__channel" href={channel.href}>
                <ChannelIcon kind={channel.kind} glyph={channel.glyph} />
                <span className="contact__text">
                  <span className="contact__label">{channel.label}</span>
                  {channel.detail != null &&
                    channel.detail.toLowerCase() !==
                      channel.label.toLowerCase() && (
                      <span className="contact__detail">{channel.detail}</span>
                    )}
                </span>
              </a>
            </li>
          ))}
        </ul>
      )}
    </Section>
  )
}
